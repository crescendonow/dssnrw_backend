...
//sum result from pipe id input ..
$sql_pipesum = "SET CLIENT_ENCODING TO 'utf-8'; 
...
$pipesum_result = pg_exec($connection, $sql_pipesum);

$numrows = pg_numrows($pipesum_result);
...
if (!$pipesum_result) {
    ...
    $data['status'] = 'true';
    $data['message'] = 'complete for pipe_sum from pipe id.';

    for ($ri = 0; $ri < $numrows; $ri++) {
        $row = pg_fetch_array($pipesum_result, $ri);
        $data['pipe_sum'] = array(
            'c_pipe' => $row['c_pipe'],
            'l_pipe' => $row['l_pipe'],
            'c_leak' => $row['c_leak'],
...
//summary pipe type and size from pipe id input 
$sql_pipe_typesize = " SET CLIENT_ENCODING TO 'utf-8'; 
...
$pipe_typesize_result = pg_exec($connection, $sql_pipe_typesize);

$numrows = pg_numrows($pipesum_result);

if (!$pipe_typesize_result) {
    $data['status'] = 'false';
    $data['message'] = json_encode(pg_last_error($db));
    $data['pipe_sum'] = array();
} else {
    ...
    $data['status'] = 'true';
    $data['message'] = 'complete for pipe_typesize from pipe id.';
    ...
    $data['pipe_typesize'] = $pipe_typesize;
}

//create all data in branch

$sql_pipeall = "SET CLIENT_ENCODING TO 'utf-8'; 
                SELECT * FROM dssnrw.data_summary 
                WHERE zone = '{$zone}' AND pwa_code = '{$pwa_code}' ";

$result_pipeall = pg_exec($connection, $sql_pipeall);

$numrows = pg_numrows($result_pipeall);
...
if (!$result_pipeall) {
    ...
    $data['status'] = 'true';
    $data['message'] = 'complete for detail of pipe all in branch.';

    for ($ri = 0; $ri < $numrows; $ri++) {
        $row = pg_fetch_array($result_pipeall, $ri);
        $data['pipe_all'] = array(
...
//pipe detail map create data. 
$sql_pipe_detailmap = "SET CLIENT_ENCODING TO 'utf-8';
...
$result_detail_map = pg_exec($connection, $sql_pipe_detailmap);

if (!$result_detail_map) {
    $data['status'] = 'false';
    $data['message'] = json_encode(pg_last_error($db));
    $data['pipe_detailmap'] = array();
} else {
    $arr = pg_fetch_array($result_detail_map, 0, PGSQL_NUM);
    $data['message'] = "Complete for return geojson by pipe id input." ;
    $data['pipe_detailmap'] = $arr;
}
...
if ($act == 'f_weight') {
    ...
    $sqlPipetypesizefreq =  " SET CLIENT_ENCODING TO 'utf-8'; 
                                    ...
                                    0 elev, 0 w_elev, 0 elev_ww, 0 ptype, 
                                    {$w_ptype} w_ptype, 
                                    {$w_ptype} * (CASE WHEN pp.pipe_type IN ('ST', 'ST_UN', 'ST_ON', 'ST_CV', 'GS', 'DI', 'CI') THEN 1 
                                        WHEN pp.pipe_type IN ('PVC', 'PVC_O', 'HDPE', 'PB', 'GRP') THEN 2 
                                        WHEN pp.pipe_type = 'AC' THEN 3 ELSE 1 END)  ptype_ww, 
                                    ...
                                    + ({$w_ptype} * (CASE WHEN pp.pipe_type IN ('ST', 'ST_UN', 'ST_ON', 'ST_CV', 'GS', 'DI', 'CI') THEN 1 
                                        WHEN pp.pipe_type IN ('PVC', 'PVC_O', 'HDPE', 'PB', 'GRP') THEN 2 
                                        WHEN pp.pipe_type = 'AC' THEN 3 ELSE 1 END))
                                    ...
                                    ON pp.pwa_code = rg.pwa_code 
                                    WHERE pp.pwa_code = '{$pwa_code}' 
                                    GROUP BY pp.pipe_type, pp.pipe_size, pp.yearinstall, l.repaircost, rg.r_score
                                    ORDER BY pp.pipe_type, pp.pipe_size ";
    ...
    $strPipetyesizefreq_map = " SET CLIENT_ENCODING TO 'utf-8';
                                    ...
                                    , row_to_json((SELECT l FROM (SELECT pipe_id, avg_lk, c_leak, w_leak, c_leak_ww,
                                                    ...
                                                    pressure, w_pressure, pressure_ww, w_branch, branch_ww, sum_ww, pipe_long, project_no, contrac_date,   
                                ...
                                +({$w_branch} * rg.r_score) ) sum_ww, 
                                            ...
                                            ON pp.pwa_code = rg.pwa_code 
                                            WHERE pp.pwa_code = '{$pwa_code}' AND pp.gen_geometry IS NOT NULL
                                            GROUP BY pp.pipe_id, pp.pipe_type, pp.pipe_size, pp.yearinstall, pp.pipe_long,
                                                    ...
                                                    pp.depth, pp.locate, pp.project_no, pp.gen_geometry, l.repaircost, rg.r_score
                                            ORDER BY pp.pipe_id ) As lg ) As f )  As fc  ";
    ...
    $geoSQL = "SET CLIENT_ENCODING TO 'utf-8'; 
            ...
            1) AS INTEGER) + 543 - pp.yearinstall >  {$age_data['value_2']} THEN 3 
                        ...
                        GROUP BY pt.pipe_id, pp.pipe_type, pp.pipe_size, pp.yearinstall,
                                pp.contrac_date, pp.asset_code, pp.pipe_func, pp.laying, pp.product, 
                                pp.depth, pp.locate, pp.project_no, rg.r_score
    ...
    if (!$resultmap) {
        $data['status'] = 'false';
        $data['message'] = json_encode(pg_last_error($db));
        $data['pipe_detail_freq_map'] = array();
        
    } else {
        $data['status'] = 'true';
        $data['message'] = json_encode("Complete for detail of map with pipe_id_form.");
        $arr = pg_fetch_array($resultmap, 0, PGSQL_NUM);
        $data['pipe_detail_freq_map'] = $arr[0];
    }
