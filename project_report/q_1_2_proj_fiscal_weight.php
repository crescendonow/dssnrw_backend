...
if ($act == "group_project_weight") {
    ...
    $q_project_weight = " SET CLIENT_ENCODING TO 'utf-8'; 
                            SELECT imp.zone, imp.pwa_code, imp.project_prov_name, SUM(imp.pipe_long) AS sum_long, imp.proj_cost, 
                            imp.remark, imp.uid_insert, 
            ROUND(SUM(imp.sum_ww)/COUNT(*), 2) AS avg_ww FROM 
        (SELECT pm.zone, pm.pwa_code, pm.project_prov_name, pm.pipe_id, pm.pipe_type, pm.pipe_size, pm.pipe_long, pm.proj_cost, project_prov_id, 
                            pm.remark, pm.uid_insert, 
                            ({$w_leak} * (CASE WHEN SUM(c_leak) < {$leak_data['value_1']} THEN 1 
                                    ...
                                    + ({$w_age} * (CASE WHEN  pipe_age < {$age_data['value_1']} THEN 1 
                                                            ...
                                                            ELSE 0 END)) 
                                    + ({$w_ptype} * (CASE WHEN pm.pipe_type IN ('ST', 'ST_UN', 'ST_ON', 'ST_CV', 'GS', 'DI', 'CI') THEN 1 
                                        WHEN pm.pipe_type IN ('PVC', 'PVC_O', 'HDPE', 'PB', 'GRP') THEN 2 
                                        WHEN pm.pipe_type = 'AC' THEN 3 ELSE 1 END)) +  
                                        ({$w_branch} * rg.r_score 
                            ...
                            WHERE pm.pwa_code = '{$pwa_code}' AND fiscal_year = '{$proj_fiscal}' AND budget_type = '{$proj_budget}'   
                            GROUP BY pm.zone, pm.pwa_code, pm.project_prov_name, pm.project_prov_id, pm.pipe_id, pm.pipe_type, pm.pipe_size, 
                            pm.pipe_long, ps.pipe_age, rg.r_score, ps.dma_nrw, pm.proj_cost, pm.remark, pm.uid_insert 
                            ORDER BY pm.zone, pm.pwa_code, pm.project_prov_name, pm.project_prov_id, pm.pipe_id, pm.pipe_type, pm.pipe_size ) imp 
							GROUP BY imp.zone, imp.pwa_code, imp.project_prov_name, imp.proj_cost, imp.remark, imp.uid_insert  ";
...
if ($act == "group_pipetype_weight") {
    ...
    $q_pipe_type_weight = "  SET CLIENT_ENCODING TO 'utf-8'; 
                             SELECT imp.zone, imp.pwa_code, imp.project_prov_name, SUM(imp.pipe_long) AS sum_long, imp.pipe_type, imp.pipe_size, 
                             imp.remark, imp.uid_insert,  
                ...
                pm.remark, pm.uid_insert,  
                        ...
                        GROUP BY pm.zone, pm.pwa_code, pm.project_prov_name, pm.project_prov_id, pm.pipe_id, pm.pipe_type, pm.pipe_size, 
                            pm.pipe_long, ps.pipe_age, rg.r_score, ps.dma_nrw, pm.proj_cost, pm.remark, pm.uid_insert  
                                    ORDER BY pm.zone, pm.pwa_code, pm.project_prov_name, pm.project_prov_id, pm.pipe_id, pm.pipe_type, pm.pipe_size ) imp 
                                                    GROUP BY imp.zone, imp.pwa_code, imp.project_prov_name, imp.pipe_type, imp.pipe_size, imp.remark, imp.uid_insert 
                                                    ORDER BY imp.project_prov_name, imp.pipe_type, imp.pipe_size       ";

    $result = pg_exec($connection, $q_pipe_type_weight);

    //echo $q_pipe_type_weight;
    //exit() ;
    $numrows = pg_numrows($result);
...
if ($act == "group_pipeid_weight") {
    ...
    $q_pipe_id_weight = " SET CLIENT_ENCODING TO 'utf-8'; 
                        SELECT pm.zone, pm.pwa_code, pm.project_prov_name, pm.pipe_id, pm.pipe_type, pm.pipe_size, pm.pipe_long, 
                        pm.remark, pm.uid_insert, 
                    ({$w_leak} * (CASE WHEN SUM(c_leak) < {$leak_data['value_1']} THEN 1 
                            WHEN  SUM(c_leak) >= {$leak_data['value_1']}
                                    AND SUM(c_leak) <= {$leak_data['value_2']}  THEN 2
                    ...
                    (SELECT pwa_code, r_score FROM dssnrw.pwa_risk_group) rg
                            ON ps.pwa_code = rg.pwa_code 
                    WHERE pm.pwa_code = '{$pwa_code}' 
										GROUP BY pm.zone, pm.pwa_code, pm.project_prov_name, pm.project_prov_id, pm.pipe_id, pm.pipe_type, 
                                        pm.pipe_size, pm.pipe_long, ps.pipe_age, rg.r_score, ps.dma_nrw, pm.remark, pm.uid_insert 
                    ORDER BY pm.zone, pm.pwa_code, pm.project_prov_name, pm.project_prov_id, pm.pipe_id, pm.pipe_type, pm.pipe_size  ";

    $result = pg_exec($connection, $q_pipe_id_weight);

    //echo $q_pipe_id_weight;
    $numrows = pg_numrows($result);
