...
if ($act == 'group_project') {
    ...
    $q_project = "  SET CLIENT_ENCODING TO 'utf-8'; 
                    ...
                    WHERE rm.budget_type = '{$proj_budget}' AND rm.fiscal_year = '{$proj_fiscal}' AND rm.approve_status = 1  
                        AND rm.zone = '{$zone}' AND rm.pwa_code = '{$pwa_code}' 
                        ORDER BY rm.project_prov_id ";

    $result = pg_query($connection, $q_project);
...
if ($act == 'group_pipetype') {
    ...
    $q_pipe_type = "    SET CLIENT_ENCODING TO 'utf-8'; 
                        SELECT pm.zone, pm.pwa_code, pm.project_prov_id, pm.project_prov_name, pipe_type, pipe_size, SUM(pipe_long) AS sum_long, 
                        pm.proj_cost, pm.remark, pm.uid_insert 
                        FROM dssnrw.pipe_improve pm
                        LEFT JOIN dssnrw.ref_pipe_improve rm 
                        ON pm.project_prov_id = rm.project_prov_id 
                        WHERE pm.pwa_code = '{$pwa_code}' AND rm.budget_type = '{$proj_budget}' AND rm.fiscal_year = '{$proj_fiscal}' AND rm.approve_status = 1 
                        GROUP BY pm.zone, pm.pwa_code, pm.project_prov_name, pipe_type, pipe_size, pm.project_prov_id, pm.remark, pm.uid_insert, pm.proj_cost 
                        ORDER BY pm.project_prov_id, pipe_type, pipe_size ";
...
if ($act == 'group_pipeid') {
    ...
    $q_pipe_id = "SET CLIENT_ENCODING TO 'utf-8'; 
                    ...
                    ON pm.project_prov_id = rm.project_prov_id 
                    WHERE pm.pwa_code = '{$pwa_code}' AND rm.budget_type = '{$proj_budget}' AND rm.fiscal_year = '{$proj_fiscal}' AND rm.approve_status = 1
                    ORDER BY pm.project_prov_id, pipe_id, pipe_type, pipe_size ";
